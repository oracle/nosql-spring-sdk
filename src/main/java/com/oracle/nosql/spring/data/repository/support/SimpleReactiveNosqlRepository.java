...
public class SimpleReactiveNosqlRepository <T, ID extends Serializable>
        ...
        return Mono
            ...
            .flatMap(ent ->
            {
                if (entityInformation.isAutoGeneratedId()) {
                    ...
                    boolean isNew = entityInformation.isNew(ent);
                    if (isNew) {
                        return nosqlOperations.insert(entityInformation,
                            ent);
                    } else {
                        return nosqlOperations.update(entityInformation,
                            ent);
                    }
                } else {
                    // do a put (set or insert) if !isAutoGeneratedId
                    return nosqlOperations.insert(entityInformation, ent);
                }
            });
    }
    ...
    public Mono<Void> delete(T entity) {
        ...
        return Mono
            .just(entityInformation.getId(entity))
            .flatMap(id -> nosqlOperations.deleteById(entityInformation, id))
            .then();
    }
    ...
    public Flux<T> findAll(Sort sort) {
        ...
        return Mono
            .just(new CriteriaQuery(Criteria.getInstance(CriteriaType.ALL),
                nosqlOperations.getConverter().getMappingContext()).with(sort))
            .cast(NosqlQuery.class)
            .flatMapMany( query -> nosqlOperations.find(query, entityInformation));
    }
